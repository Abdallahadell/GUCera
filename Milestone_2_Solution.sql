...
AS
SELECT DISTINCT Course.name FROM Course LEFT OUTER JOIN StudentTakeCourse ON Course.id = StudentTakeCourse.cid 
...
BEGIN
IF (EXISTS(SELECT * FROM Users WHERE id=@Sid))
SELECT DISTINCT A.* FROM Assignment A INNER JOIN Course C 
